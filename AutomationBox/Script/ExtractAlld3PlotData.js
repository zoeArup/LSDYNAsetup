...
try{
	...
	Message("Login config json inside the same folder")
	var settingFile = new File(fullPath+"\\config.json", File.READ);
}

var data=JSON.parse(settingFile.ReadAll());
var caseToRun=data.caseToRun;
var outputConfig=data[caseToRun].OutputConfig;
var timestepSkip=outputConfig.TimeStepOfOutput
var Debris=outputConfig.SoilExtra
...
posFiles[2] = setupSolidFiles('Solid_Extra_'+Debris+'.csv');
...
function saveFlowDepth() {
	for (var kk = 1; kk < nstate; kk += timestepSkip) {
		...
		for (var bb = 1; bb < nsolid - 1; bb++) {
			var ALE_properties = GetData(SOX, SOLID, bb, 0, Debris);

			posFiles[2].Write(
				ALE_properties + ",");
		}
		var ALE_properties = GetData(SOX, SOLID, nsolid - 1, 0, Debris);
		posFiles[2].Write(
			ALE_properties + "\n");
