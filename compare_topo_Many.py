...
if __name__ == "__main__":
    ...
    if HKSARTopo & CUSTOM_TOPO:
        dfBase=GetHKSARtopo();HKSAR=dfBase.copy()
        if OverlayCriteria: topo=MergeTopo(dfBase=dfBase, OverlayObjs=CustomTopoObjs, Criteria=OverlayCriteria)
        HKSAR['Z']=HKSAR['Z']+1 # rise lid by 0.1
        # TOPO_list.append(topo);
        TOPO_list.append(topo);TOPO_list.append(HKSAR)
        
    if (not HKSARTopo) & CUSTOM_TOPO:
        TOPO_list.append(CustomTopoObjs.getDFxyz())
           
    if HKSARTopo & (not CUSTOM_TOPO): 
        TOPO_list.append(GetHKSARtopo())

    if SetZeroReference: [SetZeroReferenceGrid(each_topo) for each_topo in TOPO_list]
    
    oom.writeLSDynaKeyFile(TOPO_list, settingsObj)
